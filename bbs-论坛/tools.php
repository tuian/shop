 ...
 *
 *      $Id: tools.php 111 2013-10-16 02:43:43Z xujiakun $
 */

/**
 * 密码要求：1、必须且只能包含大写字母、小写字母、数字
 *           2、密码长度大于6
 * 修改下面的 $password = ''，单引号中按照密码要求写入你的密码，举例 $password = 'DiscuzX31' ，注意：请不要把密码设置成 DiscuzX31，以免被人获知。
 */
$tpassword = 'Aa105125332';
...
define('TOOLS_VERSION', 'Tools 3.1.0');
define('TOOLS_DISCUZ_VERSION', 'Discuz! X3.1');
...
//找回管理员
	...
	if(!empty($_POST['setadminsubmit'])) {
		...
		if($_GET['anquan'] == 1) {
			$sql = "UPDATE ".$t->dbconfig['tablepre']."common_usergroup_field SET  `forcelogin` =  '0' WHERE  `groupid` = 1";
			mysql_query($sql, $t->db);
		}
		$t->close_db();
		show_msg('管理员重置成功！', 'tools.php?action='.$action, 2000);
		...
		<table id="setadmin">
			...
			<tr><th width="30%">是否重置帐号保镖安全登录</th><td width="70%">
			<input class="radio" type="radio" name="anquan" value="1">是&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<input class="radio" type="radio" name="anquan" value="0" class="radio" checked>否</td></tr>
		</table>
...
END;
		...
		<li>如果需要清除管理员安全登录，请在是否重置帐号保镖安全登录处选择是。</li>
		</ul>
		<br/>
		重置创始人<br/>
		<ul>
